...
module.exports = class GitHubClient {
    ...
    async generateOAuthToken(username, password, twoFactorAuthToken, scopes) {
        ...
        const login = await this.github.authorization.createAuthorization({
            scopes: oAuthScopes,
    ...
    async findOrg(orgName) {
        try {
            const result = await this.requestGently(this.github.orgs.get, {
                org: orgName,
            });
    ...
    async fetchAllResults(apiCall, options = {}) {
        ...
        const firstResult = await this.requestGently(apiCall, options);
        ...
        const fetchRemainingResults = async (prevResult) => {
            ...
            const result = await this.requestGently(this.github.getNextPage, prevResult);
    ...
    /**
     * @param {callback} apiCall
     * @param {...mixed} callArgs
     * @return {Promise}
     */
    async requestGently(apiCall, ...callArgs) {
        return this.bottleneck.schedule(() => apiCall(...callArgs));
    }
