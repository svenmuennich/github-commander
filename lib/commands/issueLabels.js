...
const configReader = require('../configuration/configReader');
...
commandRunner(async () => {
    ...
    await GitHubClient.sequentiallyAwaitEach(allGithubRepositories, async (githubRepository) => {
        ...
        const repoLabels = await client.fetchAllResults(client.github.issues.listLabelsForRepo, {
        ...
        await Promise.all(labelsToAdd.map(async (newLabel) => {
            ...
            return client.requestGently(client.github.issues.createLabel, {
                owner: githubOrg.login,
                repo: githubRepository.name,
                name: newLabel.name,
        ...
        await Promise.all(repoLabels.map(async (repoLabel) => {
            ...
            return client.requestGently(client.github.issues.updateLabel, {
                owner: githubOrg.login,
                repo: githubRepository.name,
        ...
        await Promise.all(repoLabelsToDelete.map(async (repoLabel) => {
            ...
            return client.requestGently(client.github.issues.deleteLabel, {
                owner: githubOrg.login,
                repo: githubRepository.name,
                name: repoLabel.name,
            });
