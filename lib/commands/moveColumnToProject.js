 ...
 * @return {Object}
 */
const indexCardContent = async (projectCards, contentResource, matcher) => {
    const content = await Promise.all(projectCards
        ...
        .map(async (card) => {
            ...
            const response = await contentResource.get({
                owner: matches[1],
                repo: matches[2],
...
commandRunner(async () => {
    ...
    if (destinationColumn) {
        ...
        const destinationColumnCards = await client.github.projects.getProjectCards({
        ...
        destinationColumn = await client.github.projects.createProjectColumn({
            name: columnName,
            project_id: destinationProject.id,
        });
    ...
    await GitHubClient.sequentiallyAwaitEach(sourceColumnCards.reverse(), async (card) => {
        ...
        await client.github.projects.createProjectCard(payload);
    });
});
