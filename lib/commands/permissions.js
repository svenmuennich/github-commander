...
commandRunner(async () => {
    ...
    const allGithubTeams = await client.fetchAllResults(client.github.teams.list, {
        org: githubOrg.login,
    });
    ...
    await GitHubClient.sequentiallyAwaitEach(allGithubRepositories, async (githubRepository) => {
        ...
        if (program.clearCollaborators === true && (!configRepository || configRepository.clearCollaborators !== false)) {
            ...
            const repositoryCollaborators = await client.fetchAllResults(client.github.repos.listCollaborators, {
        ...
        return Promise.all(config.teams.map((configTeam) => {
            ...
            if (repositoryPermission !== null) {
                ...
                return client.requestGently(client.github.teams.addOrUpdateRepo, {
            ...
            return client.requestGently(client.github.teams.checkManagesRepo, {
                team_id: configTeam.githubTeam.id,
                owner: githubOrg.login,
                repo: githubRepository.name,
            }).then(() => client.requestGently(client.github.teams.removeRepo, {
                team_id: configTeam.githubTeam.id,
                owner: githubOrg.login,
                repo: githubRepository.name,
            })).catch((err) => {
