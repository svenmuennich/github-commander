...
commandRunner(async () => {
    ...
    const projectColumns = await client.fetchAllResults(client.github.projects.listColumns, {
    ...
    const projectCardPromises = projectColumns.map(anyColumn => client.fetchAllResults(client.github.projects.listCards, {
    ...
    const repositoryIssues = await client.fetchAllResults(client.github.issues.listForRepo, {
        owner: orgName,
        repo: repositoryName,
        state: 'open',
    });
    ...
    await GitHubClient.sequentiallyAwaitEach(newRepositoryIssues, async (issue) => {
        let card = await client.github.projects.createCard({
            column_id: column.id,
            content_id: issue.id,
            content_type: 'Issue',
        });
        card = card.data;
        await client.github.projects.moveCard({
            id: card.id,
            position: 'bottom',
        });
    });
